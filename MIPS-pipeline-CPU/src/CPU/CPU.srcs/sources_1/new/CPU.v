...
module CPU(clk, reset, cathodes, ans);
    input clk;
    input reset;
    output [7:0] cathodes;
    output [3:0] ans;
    ...
    assign write_en =   ~(EX_MEM_ALUOut[30] == 1'b1);
    ...
    begin
        ...
        begin
            bcd_en <= 1'b1;
            clk_num <= EX_MEM_Data_In[7:0];
        end
    end
    
    BCD7 bcd7(.scan_clk(clk), .din0(clk_num), .din1(clk_num), .din2(clk_num), .din3(clk_num), .dout(cathodes), .AN(ans), .bcd_en(bcd_en));
